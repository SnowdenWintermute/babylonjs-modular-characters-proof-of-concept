...
export function getTransformNodeByName(
  sceneResult: ISceneLoaderAsyncResult,
  name: string
) {
...
export function disposeAsyncLoadedScene(
  sceneResult: ISceneLoaderAsyncResult | null
) {
  ...
  while (sceneResult.transformNodes.length)
    sceneResult.transformNodes.pop()!.dispose();
  ...
  while (sceneResult.spriteManagers.length)
    sceneResult.spriteManagers.pop()!.dispose();
  while (sceneResult.animationGroups.length)
    sceneResult.animationGroups.pop()!.dispose();
  while (sceneResult.particleSystems.length)
    sceneResult.particleSystems.pop()!.dispose();
}
